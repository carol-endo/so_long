...
int	checa_teclas(int tecla, tipo_ponteiros *ponteiros) {
	if (tecla == ESC) {
		...
		mlx_destroy_window(ponteiros->mlx, ponteiros->janela); //dá free
		mlx_destroy_display(ponteiros->mlx); //não dá free
		free(ponteiros->mlx);
		exit(0);
	}
	else
...
int main() {
	...
	ponteiro_da_janela = mlx_new_window(ponteiro_da_mlx, 600, 300, "Janela");
